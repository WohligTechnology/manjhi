...
.controller('CreateArtworkCtrl', function($scope, TemplateService, NavigationService, $upload, $timeout, $http) {
    ...
    $scope.setSearch = function(select) {
        ...
        $scope.artwork.user = select.selected._id;
    }
    ...
    NavigationService.lastSr(function(data, status) {
        $scope.artwork.srno = parseInt(data[0].count) + 1;
    });
    ...
    $scope.start = function(index, whichone) {
        ...
        if ($scope.howToSend == 1) {
            $scope.upload[index] = $upload.upload({
                url: 'http://smartsnap.in/user/uploadfile',
...
.controller('RegisterArtistCtrl', function($scope, TemplateService, NavigationService) {
    ...
    $scope.user.accesslevel = "customer";
...
.controller('TermConditionCtrl', function($scope, TemplateService, NavigationService, cfpLoadingBar, $state) {
    //Used to name the .html file
    ...
    NavigationService.getuserprofile(function(data) {
        console.log(data);
        if (data.id) {
            NavigationService.getoneartist(data.id, function(data, status) {
                $scope.userData = data;
            })
        }
    });
